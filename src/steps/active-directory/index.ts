...
export async function buildMachineManagesDevicesRelationships(
  ...
  if (!dataMachineEntity) {
    logger.warn('Error mapping machine to user endpoint: Machine Entity does not exist');
    return;
  }
  ...
  await jobState.iterateEntities(
    ...
    async (machine) => {
      await jobState.addRelationship(
        createMachinesDeviceRelationship(machine)          
      )
    },
  );
}
...
export async function fetchLogonUsers(
  ...
  await jobState.iterateEntities(
    { _type: entities.MACHINE._type },
    async (machineEntity) => {
  await graphClient.iterateUsers({ machineId: machineEntity.id as string },async (user) => {
...
export async function buildFindingIsCveRelationships(
  ...
  await jobState.iterateEntities(
    ...
    async (findingEntity) => {
      if(findingEntity.id?.toString().toLowerCase().indexOf("cve")){
        await jobState.addRelationship(
